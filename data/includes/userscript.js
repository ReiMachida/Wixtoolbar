...
function Redirector(){
    ...
    $(document).on('mouseenter','.wix-link', function(e){
        self.setEventTarget(e.target)
        console.log("option: " + WixU.option.get('Main', 'thumbnail_on'));
        if(WixU.option.get('Main', 'thumbnail_on')){
        	self.show();
        }
    })

    $(document).on('mouseleave','.wix-link', function(e){
        self.setEventTarget(e.target)
        self.hide();
    })
    ...
    this.show = function(){
        ...
        var abst = document.createElement("div");
        var img = new Image();

        
        $(thumbnail).css('background-color', 'white');

        thumbnail.id = "wixthumbnail";
        abst.id = "wixthumbnailAbst";
        img.id ="wixthumbnailImg";
        thumbnail.appendChild(img);
        thumbnail.appendChild(abst);


        if(_wtid){
            var keyword = Util.escapeUTF8(_eventTarget.innerHTML);
            thumbnail.src = (Constant.URL.Redirector + 'thumbnail?keyword=' + keyword + '&wtid=' + _wtid);
            //window.open(_broadAttachPath + keyword);
        }else{
            var wid = _eventTarget.getAttribute('wid'),
            eid = _eventTarget.getAttribute('eid');
            if(wid && eid){
                $.get(Constant.URL.Redirector + 'thumbnail?wid=' + wid + '&eid=' + eid,
                    function(data){
                        console.log(data);
                        $('#wixthumbnailImg').attr('src',data.thumbnail);
                        $('#wixthumbnailAbst').html(data.abst);
                    },'json');
                // thumbnail.src = Constant.URL.Redirector + 'thumbnail?wid=' + wid + '&eid=' + eid;
                // thumbnail.src = "https://www.pets4homes.co.uk/images/articles/1646/large/kitten-emergencies-signs-to-look-out-for-537479947ec1c.jpg";
        ...
        $('#wixthumbnail').on('error',function(){
            $('#wixthumbnail').remove();
            console.log('making Iframe');
            thumbnail = document.createElement('iframe');
            if(_wtid){
                var keyword = Util.escapeUTF8(_eventTarget.innerHTML);
                thumbnail.src = (Constant.URL.Redirector + '?keyword=' + keyword + '&wtid=' + _wtid);
                    //window.open(_broadAttachPath + keyword);
                }else{
                    var wid = _eventTarget.getAttribute('wid'),
                    eid = _eventTarget.getAttribute('eid');
                    if(wid && eid){
                        thumbnail.src = Constant.URL.Redirector + '?wid=' + wid + '&eid=' + eid;
                        // thumbnail.src = "https://www.pets4homes.co.uk/images/articles/1646/large/kitten-emergencies-signs-to-look-out-for-537479947ec1c.jpg";
                ...
                l_x = $(_eventTarget).offset().left;
                l_w = $(_eventTarget).width();
                t_w = $(thumbnail).width();
                t_x = l_x + (l_w - t_w)/2;

                console.log("l_x: " + $(_eventTarget).offset().left);
                console.log("l_w: " + $(_eventTarget).width());
                console.log("t_w: " + t_w);
                ...
                if(t_w <= windowRight){
                   if(t_x + t_w > windowRight){
                    $(thumbnail).css('left', windowRight - t_w);
                }else if (t_x < windowLeft){
                    $(thumbnail).css('left', windowLeft);
                }else{
                    $(thumbnail).css('left', t_x);
                }
            }else{
                $(thumbnail).width(windowRight);
                $(thumbnail).css('left', windowLeft);
            }
            ...
            t_y = $(_eventTarget).offset().top + $(_eventTarget).height();
            if(t_y + $(thumbnail).height() >= windowBottom){
               t_y = $(_eventTarget).offset().top - $(thumbnail).height();
           }
           $(thumbnail).css('top', t_y);

       });

        $('#wixthumbnailImg').on('load',function(){
            var imgW = $(thumbnail).width();
            var imgH = $(thumbnail).height();
    ...
    option : (function(){
        ...
        return {
            ...
            get : function(objName, key){
            	console.log("_conditions[objName][key]:" + _conditions[objName][key]);
...
WixU.main = new WixUClass({
    ...
    option : {
        ...
        thumbnail_on : function(){
        	return $('#wix-thumbnail-on').prop("checked");
        },
    },
    function : {
        ...
        setEventListener : function(){
            ...
            $('#wix-thumbnail-on').change(function(){
                console.log("set thumbnail_on to: " + $(this).prop("checked"));
                connection.send(
                    'SetMainOption',
                    {'thumbnail_on' : $(this).prop("checked")}
                    )
